...
public class UsuarioDAO {
	...
	public Usuario guardar(Usuario usuario) throws SQLException {
		...
		try {
			...
			PreparedStatement pst = c.prepareStatement(
					"INSERT INTO usuario (apellido,nombre,mail,nick,password,expira) " + "VALUES (?,?,?,?,?,?)");
	...
	public boolean modificar(Usuario usuario) throws SQLException {
		...
		try {
			c.setAutoCommit(false);
			PreparedStatement pst = c.prepareStatement(
					"UPDATE usuario SET apellido=?,nombre=?,mail=?,nick=?,password=?" + " WHERE idUsuario=?");
			pst.setString(1, usuario.getApellido());
			pst.setString(2, usuario.getNombre());
			pst.setString(3, usuario.getMail());
			pst.setString(4, usuario.getNick());
	...
	public boolean eliminar(Usuario usuario) throws SQLException {
		...
		try {
			c.setAutoCommit(false);
			PreparedStatement pst = c.prepareStatement("DELETE FROM usuario WHERE idUsuario=?");
	...
	public Usuario cargar(int idUsuario) throws SQLException {
		...
		try {
			...
			ResultSet rs = pst.executeQuery();
			// int idUsuario, String apellido, String nombre, String mail,
			// String nick, String password, Date expira
			if (rs.next()) {
	...
	public Usuario cargar(String nick) throws SQLException {
		Usuario resultado = null;

		try {
			PreparedStatement pst = c.prepareStatement("SELECT * FROM usuario WHERE nick=?");
			pst.setString(1, nick);
			ResultSet rs = pst.executeQuery();
			if (rs.next()) {
				resultado = getUsuarioFromResultSet(rs);
			}
	...
	private Usuario getUsuarioFromResultSet(ResultSet rs) throws SQLException {
		Usuario u = new Usuario(rs.getInt("idUsuario"), rs.getString("apellido"), rs.getString("nombre"),
				rs.getString("mail"), rs.getString("nick"), rs.getString("password"), rs.getDate("expira"));
		return u;
	}
	...
	public List<Grupo> listarGrupos(int idUsuario) throws SQLException {
		...
		try {
			PreparedStatement pst = c.prepareStatement(
					"SELECT g.idGrupo, g.nombre, g.descripcion" + " FROM grupo as g, grupo_x_usuario as gxu"
							+ " WHERE g.idGrupo=gxu.idGrupo" + " AND gxu.idUsuario=?");
			pst.setInt(1, idUsuario);
			ResultSet rs = pst.executeQuery();
			...
			while (rs.next()) {
				resultado.add(new Grupo(rs.getInt("idGrupo"), rs.getString("nombre"), rs.getString("descripcion"),
						gDAO.listarPermisos(rs.getInt("idGrupo"))));
			}
		} catch (Exception e) {
	...
	public List<Permiso> listarPermisosDirectos(int idUsuario) throws SQLException {
		List<Permiso> resultado = new ArrayList<Permiso>();
		try {
			PreparedStatement pst = c.prepareStatement(
					"SELECT p.idPermiso,p.nombre,p.descripcion" + " FROM usuario_x_permiso as uxp, permiso as p"
							+ " WHERE p.idPermiso=uxp.idPermiso" + " AND uxp.idUsuario=?");
			pst.setInt(1, idUsuario);
			ResultSet rs = pst.executeQuery();
			while (rs.next()) {
	...
	public boolean tieneGruposAsociados(int idUsuario) {

		boolean resultado = false;

		try {
			PreparedStatement pst = c.prepareStatement("SELECT * FROM grupo_x_usuario WHERE idUsuario=?");
			pst.setInt(1, idUsuario);
	...
	public boolean tienePermisosAsociados(int idUsuario) {
		boolean resultado = false;

		try {
			PreparedStatement pst = c.prepareStatement("SELECT * FROM usuario_x_permiso WHERE idUsuario=?");
			pst.setInt(1, idUsuario);
	...
	public boolean asignarPermiso(int idUsuario, int idPermiso) {

		boolean resultado = false;

		try {
			PreparedStatement pst = c.prepareStatement("INSERT INTO grupo_x_usuario (idPermiso,idUsuario) VALUES (?,?)");
